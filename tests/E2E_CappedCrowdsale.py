...
class MyTest(unittest.TestCase):
    def test_cap(self):
        ...
        #Invest
        tether.instance.approve(capped_crowdsale.address,price_per_token * 5, transact={'from': w3.eth.accounts[1]})
        capped_crowdsale.instance.invest(price_per_token * 5,tether.address, transact={'from': w3.eth.accounts[1]})

        with self.assertRaises(ValueError):
            tether.instance.approve(capped_crowdsale.address,price_per_token * 10, transact={'from': w3.eth.accounts[1]})
            capped_crowdsale.instance.invest(price_per_token * 10,tether.address, transact={'from': w3.eth.accounts[1]})

        tether.instance.approve(capped_crowdsale.address,price_per_token * 5, transact={'from': w3.eth.accounts[1]})
        capped_crowdsale.instance.invest(price_per_token * 5,tether.address, transact={'from': w3.eth.accounts[1]})
        
        with self.assertRaises(ValueError):
            tether.instance.approve(capped_crowdsale.address,price_per_token * 1, transact={'from': w3.eth.accounts[1]})
            capped_crowdsale.instance.invest(price_per_token * 1,tether.address, transact={'from': w3.eth.accounts[1]})

        self.assertEqual(base_token.instance.balanceOf(w3.eth.accounts[1]), 10)
        self.assertEqual(tether.instance.balanceOf(w3.eth.accounts[0]), price_per_token * 10)
